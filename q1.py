...
def rep_with_upper(final_password):
    random_index = random.randrange(len(final_password))
    random_character = final_password[random_index]
    for i in range(len(final_password)):
        if final_password[i] == random_character:
            final_password = final_password[:i] + final_password[i].upper() + final_password[i + 1:]
    print("the randomly selected character is : " + final_password[random_index])
    return final_password


upperPass = rep_with_upper(password)
print("password after randomly replacing character with its upper case:  " + upperPass)

print("****************part2.2******************")


def swap_letters(rep_with_upper_result):
    first_two_letters = rep_with_upper_result[:2]
    l = len(rep_with_upper_result)
    last_two_letters = rep_with_upper_result[l - 2:]
    fixed_letters = rep_with_upper_result[2:l - 2]
    replaced_version = last_two_letters + fixed_letters + first_two_letters
    return replaced_version


swappedVersion = swap_letters(upperPass)
print("password before  replacing first two letters with last two letters :" + upperPass)
print("password after  replacing first two letters with last two letters :" + swappedVersion)
print("****************part3******************")
...
print(search_letter(swappedVersion, "k"))
if (search_letter(swappedVersion, "k")):
